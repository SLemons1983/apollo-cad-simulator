...
export async function POST(request: Request) {
  ...
  if (!event.radioIdentifier) return NextResponse.json({ok:false,error:"Invalid emergency payload"},{status:400});
  cadIntegrationStore.emergencyByUnit.set(event.radioIdentifier,event);
